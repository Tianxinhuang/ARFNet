...
def merge_layer(rawpts,newpts,decfactor,knum=16):
    ...
    return refine_pts
def mlp_merge_layer(scope,rawpts,newpts,mlp=[64,64]):
    npoint=newpts.get_shape()[1].value
    dist1, idx1, dist2, idx2 = tf_nndistance.nn_distance(rawpts, newpts)
    #_,idx = tf_grouping.knn_point(knum, rawpts, newpts)
    #else:
    ...
    #print(grouped_xyz,newpts)
    vecs=grouped_xyz-tf.expand_dims(newpts,axis=2)
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        _,newfeat=pt_mlp('merge',tf.concat([newpts,tf.squeeze(vecs,[2])],axis=-1),mlp)
    #with tf.variable_scope(scope,reuse=True):
        #_,rawfeat=pt_mlp('merge',tf.squeeze(grouped_xyz,[2]),mlp)
        #tensor=tf.concat([newfeat,rawfeat],axis=-1)
        tensor=tf.concat([tf.tile(newfeat,[1,newpts.get_shape()[1].value,1,1]),vecs,tf.expand_dims(newpts,axis=2)],axis=-1)
        for i,outchannel in enumerate([64,64]):
            tensor=conv2d('mlp_merge%d'%i,tensor,outchannel,[1,1],padding='VALID',use_bnorm=False)
        tensor=conv2d('mlp_merge_out',tensor,1,[1,1],padding='VALID',use_bnorm=False,activation_func=None)
    decfactor=tensor#tf.exp(-tf.square(tensor))
    tf.add_to_collection('decfactor',tf.reduce_mean(tf.square(decfactor)))
    #bid=tf.tile(tf.reshape(tf.range(start=0,limit=tf.shape(rawpts)[0],dtype=tf.int32),[-1,1,1,1]),[1,npoint,knum,1])
    #idx=tf.concat([bid,tf.expand_dims(idx,axis=-1)],axis=-1)
    #grouped_xyz=tf.gather_nd(rawpts,idx)
...
def full_process(pointcloud,type_pl='p'):
    ...
    points11,dstate=tf.concat([points0,points1],axis=1),tf.concat([dstate0,dstate],axis=1)
    ...
    tf.add_to_collection('points1', points11)
    ...
    points1=mlp_merge_layer('merge1',pointcloud,points11,mlp=[128,128])
    ...
    #point_input=tf.expand_dims(points1,axis=2)
    ...
    points22,dstate=decode_cell('decode_cell',codeout2,points1,dstate,None,mlp1=[128,64],up_ratio=16,state_len=128,reuse=False,use_bnorm=False)
    ...
    tf.add_to_collection('points2', points22)
    #print(points2)
    ...
    points2=mlp_merge_layer('merge2',pointcloud,points22,mlp=[128,128])
    ...
    #point_input=tf.expand_dims(points2,axis=2)
    ...
    points_final=mlp_merge_layer('merge3',pointcloud,points_final,mlp=[128,128])
    ...
    return points11,points22,points3,points_final
